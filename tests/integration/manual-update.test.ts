...
type FixtureKind = 'stable' | 'draft-only' | 'prerelease-only' | 'malformed' | 'empty' | 'http-error' | 'timeout';
...
async function probeManualUpdate(
  ...
  return app.evaluate(async ({ BrowserWindow, Menu, shell }, args) => {
      ...
      for (const item of items) {
        ...
        if (submenu && Array.isArray(submenu.items)) {
          const found = findUpdateItem(
            submenu.items as Array<{ label?: string; click?: () => void; submenu?: { items: unknown[] } }>
          );
    ...
    const fixtureFor = (fixtureKind: typeof args.kind): { ok: boolean; status: number; body: unknown } => {
  ...
  }, { kind, stableUrl: STABLE_URL, featureChunk: FEATURE_CHUNK, appPath: APP_PATH, projectRoot: PROJECT_ROOT });
}
...
test.describe('manual update liveness', () => {
  test('imports the built feature, bounds fetch, and only opens a validated stable URL', async () => {
    ...
    try {
      ...
      const blocked: FixtureKind[] = ['draft-only', 'prerelease-only', 'malformed', 'empty', 'http-error'];
      ...
      expect(
        timedOut.snapshots.some(
          (snap) => snap.phase === 'result' && (snap.kind === 'error' || snap.message?.includes('Couldn’t'))
        )
      ).toBe(true);
