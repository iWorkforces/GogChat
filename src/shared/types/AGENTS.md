...
- `domain.ts` - app domain payload types (`OnlineCheckRequest` / `OnlineStatusData` carry `attemptId`). Do not treat leftover `PerformanceMetrics` here as the CI export contract (`performanceTypes.ts` in main owns that).
