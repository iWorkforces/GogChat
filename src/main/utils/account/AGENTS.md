...
- Shared routing/registry/bootstrap helpers live in `accountRouter.ts`, `accountWindowRegistry.ts`, `bootstrapTracker.ts`, `bootstrapWatcher.ts`, `accountSessionMaintenance.ts`, `cacheWarmer.ts`, `deepLinkUtils.ts`, and **`accountNavigation.ts`** (WebContents-first `loadAccountURL` / `getAccountURL` / `sendToAccount` — prefer these over `getAccountWindow().webContents` for navigation so WCV host is never mistaken for the account document).
